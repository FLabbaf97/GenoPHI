...
def plot_phage_umap(feature_table, select_phages, phage_col='phage', 
    ...
    try:
        ...
        raise ImportError("plot_phage_umap requires the 'umap-learn' package to be installed.") from exc
    try:
        from plotnine import ggplot, aes, geom_point, labs, theme
    except ImportError as exc:
        raise ImportError("plot_phage_umap requires the 'plotnine' package to be installed.") from exc
...
def plot_phage_clustermap(feature_table, phage_col='phage', plot_title="Phage-Phage Clustermap", save_plot=False, plot_filename='phage_clustermap.png'):
    ...
    try:
        ...
        raise ImportError("plot_phage_clustermap requires seaborn and matplotlib to be installed.") from exc
...
def cluster_phages(
    ...
    **kwargs
    ...
    """
    Cluster phages from either pangenome features or an interaction matrix.
    
    Args:
        phage_feature_table (DataFrame): DataFrame with phage features.
        method (str): Clustering method - 'HDBSCAN' or 'hierarchical'.
        top_n (int): Number of clusters for hierarchical clustering.
        feature_columns (list): Feature columns to use. If None, defaults to columns
            containing 'pc_'.
    ...
    if method == 'HDBSCAN':
        try:
            ...
            raise ImportError("cluster_phages with method='HDBSCAN' requires the 'hdbscan' package to be installed.") from exc
    ...
    if not feature_columns:
        raise ValueError(
            "No feature columns found. Pass feature_columns explicitly or ensure the "
            "feature table has columns containing 'pc_'."
        )
    ...
    if method == 'HDBSCAN':
        ...
        if verbose:
            print(f"Clustering phages using hierarchical clustering into {top_n} clusters")
        ...
        cluster_labels = fcluster(linkage_matrix, top_n, criterion='maxclust')
    else:
        raise ValueError(
            f"Unknown clustering method '{method}'. Choose from 'HDBSCAN' or 'hierarchical'."
        )
...
def cocktail_design(df, feature_table, 
    ...
    if generate_plot:
        try:
            from plotnine import ggplot, aes, labs, theme, element_text, geom_col, scale_fill_gradient, lims, facet_grid
        except ImportError as exc:
            raise ImportError("cocktail_design with generate_plot=True requires the 'plotnine' package to be installed.") from exc

    # Use a reference interaction table for training-derived promiscuity and
    # interaction-matrix clustering. If not provided, fall back to df.
    ...
    if default_cocktails:
        phage_percent = reference_df[[strain_col, phage_col, interaction_col]].drop_duplicates()
    ...
    if clustering:
        if table_type == 'interaction_matrix':
            cluster_input = (
                reference_df[[strain_col, phage_col, interaction_col]]
                .drop_duplicates()
                .pivot_table(
                    index=phage_col,
                    columns=strain_col,
                    values=interaction_col,
                    fill_value=0
                )
                .reset_index()
            )
            feature_columns = [col for col in cluster_input.columns if col != phage_col]
        elif table_type == 'pangenome':
            cluster_input = feature_table
            feature_columns = [x for x in feature_table.columns if 'pc_' in x]
            if not feature_columns:
                print("Warning: No pangenome feature columns found for clustering. Falling back to non-clustering method.")
                clustering = False
        else:
            raise ValueError(
                f"Unknown table_type '{table_type}'. Choose from 'pangenome' or 'interaction_matrix'."
            )

    if clustering:
        hdbscan_kwargs = {'min_cluster_size': 2} if clustering_method == 'HDBSCAN' else {}
        phage_clusters = cluster_phages(
            cluster_input,
...
def evaluate_cocktail_performance(
    bootstrapping_dir,
    ...
    skipped_records = []
    table_types = ['pangenome', 'interaction_matrix']
    clustering_configs = [
        ('None', False, 'hierarchical'),
        ('hierarchical', True, 'hierarchical'),
        ('HDBSCAN', True, 'HDBSCAN'),
    ]
        ...
        for (iteration, run_label), group in full_predictions_df.groupby(['iteration', 'modeling_method']):
            ...
            if not os.path.exists(performance_path):
                ...
                skipped_records.append({
                    'iteration': iteration,
                    'run_label': run_label,
                    'reason': 'missing_performance_file',
            ...
            if not os.path.exists(feature_table_path):
                ...
                skipped_records.append({
                    'iteration': iteration,
                    'run_label': run_label,
                    'reason': 'missing_feature_table',
                    'path': feature_table_path,
                })
                continue
            feature_table_df = pd.read_csv(feature_table_path)

            group = group[group['strain'].notna()].copy()
            group['strain'] = group['strain'].astype(str)

            # Only evaluate cocktails for strains represented in the current
            # predictions subset so promiscuity-based and model-based
            # strategies are compared on the same set of target strains.
            predicted_strains = set(group['strain'].dropna().unique())
            group = group[group['strain'].isin(predicted_strains)].copy()

            if group.empty:
                print('No prediction rows found for:', iteration, ' - ', run_label)
                skipped_records.append({
                    'iteration': iteration,
                    'run_label': run_label,
                    'reason': 'empty_prediction_subset',
                    'path': '',
                })
                continue

            test_strains = predicted_strains

            # Restrict the reference interaction matrix to training strains for
            # this iteration so promiscuity and interaction-matrix clustering
            # are learned only from non-held-out strains.
            training_reference_df = interaction_matrix_df[
                ~interaction_matrix_df['strain'].isin(test_strains)
            ].copy()

            if training_reference_df.empty:
                print('No training reference rows found for:', iteration, ' - ', run_label)
                skipped_records.append({
                    'iteration': iteration,
                    'run_label': run_label,
                    'reason': 'empty_training_reference',
                    'path': '',
                })
                continue
            ...
            # for n in range(1,6):
            for n in [1, 3, 5]:
                print('Running cocktail design analysis for:', iteration, 'top_n =', n)
                for selection_strategy, use_promiscuity in selection_strategies:
                    for table_type in table_types:
                        for clustering_label, clustering_flag, clustering_method in clustering_configs:
                            cocktail_design_df, cocktail_design_summary, cocktail_design_plot = cocktail_design(
                                group,
                                feature_table_df,
                                clustering=clustering_flag,
                                ...
                                plot_umap=False,
                                split_by_strain=False,
                                default_cocktails=use_promiscuity,
                                reference_df=training_reference_df
                            )
                            cocktail_design_summary['clustering'] = clustering_label
                            cocktail_design_summary['selection_strategy'] = selection_strategy
                            cocktail_design_summary['table_type'] = table_type
                            cocktail_design_summary['top_n'] = n
                            cocktail_design_summary['run_label'] = run_label
                            cocktail_design_summary['iteration'] = iteration
                            bootstrap_metrics_df = pd.concat([bootstrap_metrics_df, cocktail_design_summary])

    else:
        print('No predictions parsed')

    bootstrap_metrics_df = bootstrap_metrics_df.groupby(
        ['run_label', 'selection_strategy', 'table_type', 'clustering', 'top_n']
    ).agg({'True': 'sum', 'strain': 'count'}).reset_index()
    ...
    if skipped_records:
        print('\nSkipped iterations / missing files summary:')
        skipped_df = pd.DataFrame(skipped_records)
        display(skipped_df)
    else:
        print('\nNo iterations were skipped.')

def main():
    parser = argparse.ArgumentParser(description='Evaluate cocktail performance based on phage modeling predictions.')
    parser.add_argument('--bootstrapping_dir', type=str, required=True, help='Path to the bootstrapping directory containing iteration outputs.')
    parser.add_argument('--interaction_matrix_path', type=str, required=True, help='Path to the full observed interaction matrix CSV with strain, phage, and interaction columns.')
    ...
    bootstrapping_dir = args.bootstrapping_dir
    ...
    if os.path.exists(interaction_matrix_path):
        ...
        print(f"Interaction matrix file not found: {interaction_matrix_path}")
        return

    required_columns = {'strain', 'phage', 'interaction'}
    missing_columns = required_columns.difference(interaction_matrix_df.columns)
    if missing_columns:
        print(f"Interaction matrix is missing required columns: {sorted(missing_columns)}")
        return

    interaction_matrix_df['strain'] = interaction_matrix_df['strain'].astype(str)
    ...
    evaluate_cocktail_performance(
        bootstrapping_dir=bootstrapping_dir,
